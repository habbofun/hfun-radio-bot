...
class BattleballDatabaseService:
    ...
    Methods:
        ...
        mark_match_as_processed: Marks a match as processed in the database.
        is_match_processed: Checks if a match has been processed.
    ...
    async def mark_match_as_processed(self, match_id):
        """
        Marks a match as processed in the database.
    ...
    async def is_match_processed(self, match_id):
        """
        Checks if a match has been processed.

        Args:
            match_id (str): The ID of the match to check.

        Returns:
            bool: True if the match has been processed, False otherwise.
        """
        async with aiosqlite.connect(self.db_path) as db:
